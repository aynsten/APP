<template>
	<view>
		...
		<view class="goods-section">
			<!-- 商品列表 -->
			<view class="g-item" v-for="(item,index) in sendorderlist" :key="index">
		...
		<view class="footer">
			<view class="price-content">
				...
				<text class="price">{{totoalprice | NumToFixed}}</text>
			</view>
	...
	export default {
		...
		onShow() {
			...
			uni.getStorage({
				...
				success: function(res) {
					that.sendorderlist = res.data
					that.totoalprice = 0
					that.sendorderlist.forEach(item => {
						that.totoalprice = item.orderPrice * item.orderQty
		...
		methods: {
			submit() {
				...
				tempdata.store = that.addressData.store
				that.sendorderlist.forEach(item => {
					styles.push({
						"style": item.styleID,
